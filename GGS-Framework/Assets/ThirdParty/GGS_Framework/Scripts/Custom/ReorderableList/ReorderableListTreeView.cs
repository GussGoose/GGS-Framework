...
namespace GGS_Framework
{
	public partial class ReorderableList
	{
		public class TreeView : UnityEditor.IMGUI.Controls.TreeView
		{
			...
			{
				Dictionary<string, Rect> rects = AdvancedRect.GetRects (args.rowRect, AdvancedRect.Orientation.Horizontal,
			...
			private void DrawHeader (Rect rect)
			{
				...
				if (rl.Count > 0)
					DrawSerchBar (rects["SearchBar"]);

				if (!searchField.HasFocus () && string.IsNullOrEmpty (searchString))
					GUI.Label (rects["Label"], rl.title, Styles.header);
